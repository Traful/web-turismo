...
const Pild = (props) => <span className="bg-dark text-white p-1 rounded m-1">{props.text}</span>;
...
class PAlojamiento extends Component {
                    ...
                    .then((response) => {
                        if(response.data.data.count > 0) {
                            let fotos = response.data.data.registros.map((a, index) => {
                                return(
                                    <img key={`img-atr-${a.id}`} className="img-fluid" src={`${process.env.REACT_APP_API_RECURSOS}/imagenes/${a.imagen}`} alt="Img" onClick={(e) => {this.verImagen(e)}} />
                                );
                            });
                            self.setState({
                                fotos: fotos
    ...
    render() {
        ...
        const pilds = this.state.servicios.map((servicio) => {
            return (<Pild key={`servicio-${servicio.id}`} text={servicio.descripcion} />);
        });
        const tarifas = this.state.tarifas.map((tarifa) => {
            return(<span key={`tarifa-${tarifa.id}`} className="bg-dark text-white p-1 rounded">{tarifa.descripcion} - $ {tarifa.importe}</span>);
        });
        return (
            <div className="Alojamiento mb-5">
                {
                    ...
                    <React.Fragment>
                        <div className="ZonaLocalidad-titulo" style={{backgroundColor: `#722789`}}>
                            <h3 style={{color: `#722789`}}>{this.state.dataAlojamiento.nombre}</h3>
                        </div>
                            ...
                            <div className="row">
                                <div className="col">
                                    <div className="Alojamiento-data">
                                        ...
                                        <div className="alojamiento-fotos">
                                            {fotos}
                                        </div>
                                        <div className="alojamiento-ubicacion">
                                            <span><i className="fas fa-map-marker"></i> Ubicación</span>
                                            <div id="mapa-atr" style={{width: "100%"}}>
                                                <GoogleMap lat={this.state.dataAlojamiento.latitud} lng={this.state.dataAlojamiento.longitud} zoom="16" gwidth="100%" gheight="400px" />
                                            </div>
                                            <div className="d-flex justify-content-center p-2" style={{width: "100%"}}>
                                                <span>{this.state.dataAlojamiento.latitud} {this.state.dataAlojamiento.longitud}</span>
                                            </div>
                                        </div>
                                        <div className="alojamiento-info">
                                            <span><i className="fas fa-clock"></i> Servicios</span>
                                            <div className="d-flex flex-row flex-wrap">{pilds}</div>
                                            {
                                                tarifas.length
                                                ?
                                                <React.Fragment>
                                            ...
                                            <span className="pr-4">Domicilio: {this.state.dataAlojamiento.domicilio}</span>
                                            <span className="pr-4">Tel./Cel.: <a href={`tel:+549${this.state.dataAlojamiento.telefono}`}>+54 9 {this.state.dataAlojamiento.telefono}</a></span>
                                            <span className="pr-4">EMail: <a href={`mailto:${this.state.dataAlojamiento.mail}?Subject=Consulta`}>{this.state.dataAlojamiento.mail}</a></span>
                                            <span className="pr-4">Web: <a href={`http://${this.state.dataAlojamiento.web}`} target="_blank" rel="noopener noreferrer">{this.state.dataAlojamiento.web}</a></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
