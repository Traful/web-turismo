...
class PEvento extends Component {
        ...
        .then((response) => {
            if(response.data.data.count > 0) {
                ...
                if(registro.foto_uno) {
                    estilo = { ...estilo, backgroundImage: `url(${process.env.REACT_APP_API_RECURSOS}/recursos/eventos/${registro.foto_uno})` };
                    carousel.push(
                        <div key={`caro-uno`} className={`carousel-item ${activo}`} style={estilo}>
                ...
                if(registro.foto_dos !== "default.jpg") {
                    estilo = { ...estilo, backgroundImage: `url(${process.env.REACT_APP_API_RECURSOS}/recursos/eventos/${registro.foto_dos})` };
                    carousel.push(
                        <div key={`caro-dos`} className={`carousel-item ${activo}`} style={estilo}>
                            <h5 className="pd-top">{" "}</h5>
                        </div>
                    );
                }
    ...
    render() {
        ...
        return (
            <div className="PEvento" style={{background: "#" + this.state.evento.color}}>
                {
                    this.state.loading ?
                    <div>Cargando...</div>
                    :
                        ...
                        <div className="menu-y-slider">
                            <div id="carouselExampleIndicators" className="carousel slide" data-ride="carousel">
                                ...
                                <div className="slider-home-leyenda">
                                    <h1 className="mb-5">{this.state.evento.titulo}</h1>
                                </div>
                            </div>
                        </div>
                        <div className="container p-4">
                            <div className="row mt-3">
                                <div className="col evento-titulo"><i className="fas fa-hand-point-right"></i> {this.state.evento.titulo}</div>
                            </div>
                            <div className="row mt-2">
                                <div className="col-xs-12 col-md-6 bg-white text-dark text-uppercase text-center rounded p-2">Del {dfecha} al {hfecha} - {this.state.evento.localidad}</div>
                            </div>
                            <div className="row mt-2 pb-5">
                                <div className="col-xs-12 col-md-4">
                                    <div className="d-flex flex-column">
                                        <img className="img-fluid" src={`${process.env.REACT_APP_API_RECURSOS}/recursos/eventos/${this.state.evento.foto_uno}`} alt="Foto" />
                                        {
                                            this.state.evento.foto_dos !== "default.jpg"
                                            ?
                                            <img className="img-fluid mt-2" src={`${process.env.REACT_APP_API_RECURSOS}/recursos/eventos/${this.state.evento.foto_dos}`} alt="Foto" />
                                            :
                                            ""
                                        }
                                    </div>
                                ...
                                <div className="col">
                                    <ul className="evento-data">
                                        ...
                                        <li><i className="far fa-clock"></i> Horario</li>
                                        <li>De {dhora} hs a {hhora} hs</li>
                                        <li><i className="fas fa-dollar-sign"></i> Costo</li>
                                        <li>$ {this.state.evento.costo}</li>
                                        <li><i className="fas fa-users"></i> Invita</li>
                                        <li>{this.state.evento.invita}</li>
                                        <li><i className="fas fa-user-tie"></i> Organiza</li>
